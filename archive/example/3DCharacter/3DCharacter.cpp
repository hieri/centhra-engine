...
class game3DCharacter : public Game
{
	ActionMap<game3DCharacter> m_actionMap;
	Canvas *m_canvas;
	o3dBox *m_o3dPlane;
...
public:
	...
	{
		char title[48];
		snprintf(title, 48, "Centhra Engine v%s - 3D Character Example", getVersionString().c_str());
		print("%s\n", title);
		...
		m_actionMap.addPopAction(std::pair<unsigned int, unsigned short>(Key_F1, Mod_None), &game3DCharacter::screenshot);
...
int main(int argc, char **argv)
{
	...
	game3DCharacter *game = new game3DCharacter;
	game->start();
